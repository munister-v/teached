...
(function () {
  ...
  style.textContent = [
    ...
    '  --mob-nav-h:64px;',
    '  height:calc(var(--mob-nav-h,64px) + env(safe-area-inset-bottom,0px));',
    ...
    '  min-width:0;height:52px;gap:3px;text-decoration:none;border-radius:16px;',
    '  color:rgba(94,94,74,0.45);',
    '  transition:color 0.15s, opacity 0.15s, background 0.15s, transform 0.12s;',
    '  -webkit-tap-highlight-color:transparent;',
    '  cursor:pointer;border:none;background:none;font-family:inherit;',
    '  position:relative;',
    '}',
    '.mob-nav-tab:active{opacity:0.74;transform:scale(.96);}',
    '.mob-nav-tab.active{color:#1C1C1E;background:rgba(200,230,50,.22);}',
    '.mob-nav-icon{line-height:1;display:flex;align-items:center;justify-content:center;}',
    '.mob-nav-label{',
    '  font-size:10px;font-weight:700;letter-spacing:0.02em;',
    '  font-family:inherit;line-height:1;',
    '}',
    '@media(max-width:390px){.mob-nav{gap:1px;padding-left:4px;padding-right:4px}.mob-nav-tab{height:52px;border-radius:14px}.mob-nav-label{display:none}.mob-nav-tab.active .mob-nav-label{display:block;font-size:9px}.mob-nav-icon svg{width:21px;height:21px}}',
    /* body padding so content isn't hidden behind nav */
    'body{padding-bottom:calc(var(--mob-nav-h,64px) + env(safe-area-inset-bottom,0px) + 12px) !important;}',
  ].join('');
  ...
  window.addEventListener('pagehide', function () {
    mql.removeEventListener?.('change', toggleNav);
  }, { once: true });
})();
